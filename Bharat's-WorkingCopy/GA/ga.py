...
class GA:
    ...
    def selectPop(popSize):
        ...
        def evaluateInd(ind):
            # s = str(ind)
            # result = len(s)
            result = AssetFuzzy3.fitness(ind[0], ind[1], ind[2], ind[3],dfTrain,cash_bal)
            return result,
	...
	def CheckTrade(pd_rates):
        result_trade = AssetFuzzy3.fitness(ind[0], ind[1], ind[2], ind[3],pd_rates,cash_bal)
            return result_trade